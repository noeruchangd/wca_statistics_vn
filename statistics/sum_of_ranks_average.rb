...
class SumOfRanksAverage < Statistic
  def initialize
    @title = "Top 200 people with highest sum of national rankings (average) of Vietnamese competitors"
    @table_header = { "Person" => :left, "SoR" => :center, "333" => :center, "222" => :center, "444" => :center, "555" => :center, "666" => :center, "777" => :center, "333oh" => :center, "sq1" => :center, "minx" => :center, "pyram" => :center, "skewb" => :center, "clock" => :center, "444bf" => :center, "555bf" => :center, "333bf" => :center, "333fm" => :center }
  end
  ...
  def query
    <<-SQL
        ...
        max_rank_avg AS (
            ...
            AND ra.event_id NOT IN ('333mbo','magic','mmagic','333ft','333mbf')
        ...
        grid AS (
            ...
            JOIN events e
            WHERE p.country_id = 'Vietnam'
                AND e.id NOT IN ('333mbo','magic','mmagic','333ft','333mbf')
            AND p.sub_id = 1
        ),
        ...
        rank_avg AS (
            ...
            LEFT JOIN max_rank_avg m
                ON m.event_id = g.event_id
            WHERE g.event_id NOT IN ('333mbo','magic','mmagic','333ft','333mbf')
        ),
        ...
        sum_avg AS (
            SELECT
                person_id,
                SUM(rank_value) AS sum_rank_avg
            FROM rank_avg
            GROUP BY person_id
        )
        ...
        GROUP BY 
            s.person_id, p.name, s.sum_rank_avg

        ORDER BY s.sum_rank_avg ASC LIMIT 200;
    SQL
  end
end
