...
class DelegatedInMostConsecutiveWeeks < Statistic
  def initialize
    @title = "Most consecutive weeks with a delegated competition"
  ...
  def query
    <<-SQL
      ...
      JOIN users user ON user.id = cd.delegate_id
      JOIN persons person ON person.wca_id = user.wca_id AND person.sub_id = 1 AND person.country_id = 'Vietnam'
